...
def RL_search(surrogate_model):
    ...
    search_network = search_model(input_dim, action_dim).to(device)  # +1은 타겟 strength 포함
    optimizer = optim.Adam(search_network.parameters(), lr=1e-3, weight_decay=0.1)
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, len(train_data), batch_size):
            ...
            initial_state = np.array([init(df.iloc[:,:-1]) for i in range(features.shape[0])])

            # LightGBM 예측
            scaled_initial_state = scaler.transform(pd.DataFrame(initial_state, columns=scaler.feature_names_in_))
            predicted_strengths = surrogate_model.predict(scaled_initial_state)

            # search model 입력 구성
            search_input = np.hstack([initial_state, actual_strengths[:,np.newaxis] - predicted_strengths[:,np.newaxis]])
            search_input_tensor = torch.tensor(search_input, dtype=torch.float32).to(device)

            # 액션 예측
            predicted_actions_tensor = search_network(search_input_tensor)
            ...
            # 업데이트된 상태 계산
            updated_state = nn.ReLU()(torch.tensor(initial_state).to(device) + predicted_actions_tensor).cpu().detach().numpy()
            scaled_updated_state = scaler.transform(pd.DataFrame(updated_state, columns=scaler.feature_names_in_))
            updated_strengths = surrogate_model.predict(scaled_updated_state)
            ...
            # search model 손실 계산 (리워드 활용)
            predicted_actions_tensor *= rewards_tensor
            actual_actions_tensor *= rewards_tensor
        ...
        if val_min_reward_mean < min_rew: 
            ...
            torch.save(search_network.state_dict(), './model.pth') 
        # Epoch 결과 출력
    ...
    test(df, 7, surrogate_model, search_network, scaler, device)

def test(df, idx, surrogate_model, search_network, scaler, device):
    init_in = np.array(df.iloc[:,:-1].mean().values)
    tar_d = df.iloc[idx, :-1]
    tar = df.iloc[idx]['strength']

    now = surrogate_model.predict(scaler.transform(pd.DataFrame(init_in.reshape(1,8), columns=scaler.feature_names_in_)))
    input = np.hstack([init_in.reshape(1,8),np.array([tar])[:,np.newaxis] - now[:,np.newaxis]])
    input_tensor = torch.tensor(input, dtype=torch.float32).to(device)
    ac = search_network(input_tensor).cpu().detach().numpy()
    print(f'예측 된 액션: {ac}')
    ofe = ac+init_in.reshape(1,8)
    fe = np.maximum(0,ofe)
    print(f'최종 x:\n{pd.DataFrame(fe, columns=df.columns[:-1]).iloc[0]}')
    init_fe = scaler.transform(pd.DataFrame(init_in.reshape(1,8), columns=scaler.feature_names_in_))
    init_st = surrogate_model.predict(init_fe)
    up_fe = scaler.transform(pd.DataFrame(fe, columns=scaler.feature_names_in_))
    up_st = surrogate_model.predict(up_fe)
    print(f'목표 x:\n{tar_d}')
