...
def dynamic_encode(df: pd.DataFrame, feature_info: dict, scaler: dict) -> pd.DataFrame:
    ...
    if 'text' in feature_info:
        for col in feature_info['text']:
            ...
            # BERT 임베딩 평균값 계산
            try:
                bert_mean_embeddings = bert_encode(unique_values, bert_model, bert_tokenizer)
                bert_mean_embeddings.columns = [f"{col}_bert_mean"]  # 단일 평균값 컬럼 추가
                bert_mean_embeddings.index = bert_mean_embeddings.index.astype(str)  # 인덱스 타입 통일
